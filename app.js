...
app.post('/compose', (req,res)=>{
  const title=req.body.publish_box
  const post=req.body.postBody
  const postFull=new Post({title:title,post:post})
  postFull.save((err)=>{
    if (!err){
        res.redirect('/')
    }
...
Post.findById(postId,(err,foundPost)=>{
    res.render(__dirname+'/views/post.ejs',{title:foundPost.title,post:foundPost.post})
  } )
})
